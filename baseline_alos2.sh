...
dir_path=$(pwd)
min=-300 #for y coordinate (baseline range)
max=300
raw=$dir_path/raw
###################################################################
...
cd $raw
baseline_table.csh $super_master.PRM $master.PRM GMT > table.gmt
y_m=$(cat table.gmt | awk '{print $2}')
baseline_table.csh $super_master.PRM $slave.PRM GMT > table.gmt
y_s=$(cat table.gmt | awk '{print $2}')
cd $dir_path
master_tmp=$(echo $master | awk '{print substr($0,23,6)}')
master_full=$(echo $master_tmp | sed 's/^/20/')
slave_tmp=$(echo $slave | awk '{print substr($0,23,6)}')
slave_full=$(echo $slave_tmp | sed 's/^/20/')
year_m=$(echo "$master_full" | awk '{print substr($0,1,4)}')
month_m=$(echo "$master_full" | awk '{print substr($0,5,2)}')
day_m=$(echo "$master_full" | awk '{print substr($0,7,2)}')
year_s=$(echo "$slave_full" | awk '{print substr($0,1,4)}')
month_s=$(echo "$slave_full" | awk '{print substr($0,5,2)}')
day_s=$(echo "$slave_full" | awk '{print substr($0,7,2)}')
...
gmt psxy base_config.txt -R"$start_date"T/"$end_date"T/"$min"/"$max" -JX9i/6i -Bsx1Y -Bpxa3Of1o+l"Acqusition time" -Bpy50+l"Perpendicular Baseline (m)" -BWeSn+t"ALOS PALSAR-2 $network Network" -K -Wthinner > baseline_pair.ps
